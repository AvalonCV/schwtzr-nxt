...
const favicon_elements: any = favicon;
...
const makeWebpackScriptAssetsPreloadable = (response: Response) => {
	getWebpackScriptAssets(response)
		.forEach((path) => {
			response.setHeader('link', `<${path}>; rel=preload; as=script`);
		});
};
...
export default function serverRenderer() {
	return function(req: Request & {i18n: i18next.i18n}, res: Response, _next: NextFunction) {
		...
		renderToStringWithData(getReactApp())
			.then((content: string) => {
				...
				makeWebpackScriptAssetsPreloadable(res);
